...
5. Check the zone's bot protection and Security Events. Standard **Bot Fight Mode cannot exempt a hostname or path** and can challenge legitimate Alexa server requests. A WAF Skip rule, Page Rule, or changing Browser Integrity Check does not bypass it. Use an endpoint with suitable existing protection or a separately approved configuration supporting scoped exceptions, such as Super Bot Fight Mode. Do not disable a shared zone's Bot Fight Mode or allow broad Amazon IP ranges as an incidental installation step. This prerequisite is checked by the operator, not the planning helper.
...
Those probes use the operator's network and client, so they cannot prove Cloudflare will admit Amazon traffic. If Alexa reports `403` or an invalid endpoint response, inspect Security Events for the exact hostname/path and test timestamp before changing the application. Distinguish `botFight` from Browser Integrity Check, managed WAF rules and custom rules. An operator IP allowlist can skip custom rules for the probe while Amazon still faces other protections.
...
References: [Cloudflare ingress matching and path regular expressions](https://developers.cloudflare.com/tunnel/advanced/local-management/configuration-file/), [routing DNS to a tunnel](https://developers.cloudflare.com/cloudflare-one/networks/connectors/cloudflare-tunnel/routing-to-tunnel/), [Bot Fight Mode limitations](https://developers.cloudflare.com/bots/get-started/bot-fight-mode/), [handling bot protection false positives](https://developers.cloudflare.com/bots/troubleshooting/false-positives/), [Amazon HTTPS hosting requirements](https://developer.amazon.com/en-US/docs/alexa/custom-skills/host-a-custom-skill-as-a-web-service.html).

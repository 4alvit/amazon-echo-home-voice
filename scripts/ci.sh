...
cd "$(dirname "$0")/.."
mode="${1:-all}"
case "$mode" in syntax|unit|container|all) ;; *) echo 'Usage: scripts/ci.sh [syntax|unit|container|all]' >&2; exit 2 ;; esac
...
if [[ "$mode" == container || "$mode" == all ]]; then
  ...
  docker run --rm --volume "$PWD/tests:/tests:ro" --entrypoint python home-energy-test -m unittest discover -s /tests -v
fi
